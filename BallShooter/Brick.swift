...
class Brick {
    ...
    init(value: Int, placement: Int, size: CGFloat, categoryBitMask: UInt32, contactTestBitMask: UInt32, collisionBitMask: UInt32) {
...
//        self.node.position.y = self.yPosition
//        print(self.node.position.y)
        //self.valueLabel.position = CGPoint(x: valueLabel.position.x, y: self.yPosition + (size / 2) - 11)
    }
